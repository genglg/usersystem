...
<body class="navbar-top">
		...
		<div class="container-fluid main-content">
			<div class="widget-container fluid-height clearfix">
				<div class="heading clearfix">
					<i class="icon-table"></i> 班级管理 <a id="add-row"
					...
					<div id="w0" class="grid-view">
						<div class="table-responsive">
							<table class="table table-hover">
								...
								<tbody>
									<c:forEach items="${listCustomer.list}" var="customerss">
										<tr data-key="13">
											...
											<td>${customerss.customerName }</td>
											<td>${customerss.fieldName }</td>
											<td>${customerss.sourceName}</td>
											<td><fmt:formatDate value="${customerss.createTime}"
													pattern="yyyy-MM-dd HH:mm" />
											</td>
											<td><fmt:formatDate value="${customerss.updateTime}"
													pattern="yyyy-MM-dd HH:mm" />
											</td>
											...
											<td>
												<div>
													<button type="button" " name="updatebtn"
														...
														id="${customerss.customerId}_ct"
														onclick="checkCustomer(this.id)">查看</button>
													<button type="button" " name="updatebtn"
														class="btn btn-warning btn-sm"
														id="${customerss.customerId}_xg"
														onclick="updateCustomer(this.id)">留言</button>
												</div></td>
										</tr>
									</c:forEach>
								</tbody>
	...
	<script type="text/javascript">
		function checkCustomer(id) {
			...
			location.href = 'CustomerServlet?method=selectByPrimaryKey&customerId='
					+ customerId + '&type=1';
		}
